...
public class Utility {
	  ...
	  * @return 페이징 생성 문자열
	  */ 
	 public static String paging(int totalRecord, int nPage, int recordPerPage,String url,int carpoolno,int nowPage, String kind1,String kind2, String word1,String word2){ 
...
//	     str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	   ...
	   if (nowGrp >= 2){ 
	     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&kind1="+kind1+"&kind2="+kind2+"&word1="+word1+"&word2="+word2+"&carpoolno="+carpoolno+"&nPage="+_nowPage+"'>이전</A></span>"); 
	   } 
	   ...
	   for(int i=startPage; i<=endPage; i++){ 
	     ...
	     if (nPage == i){ 
	       ...
	       str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&kind1="+kind1+"&kind2="+kind2+"&word1="+word1+"&word2="+word2+"&carpoolno="+carpoolno+"&nPage="+i+"'>"+i+"</A></span>");   
	     } 
	   } 
	    
	   _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
	   if (nowGrp < totalGrp){ 
	     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&kind1="+kind1+"&kind2="+kind2+"&word1="+word1+"&word2="+word2+"&carpoolno="+carpoolno+"&nPage="+_nowPage+"'>다음</A></span>"); 
	   } 
	   str.append("</DIV>"); 
	 ...
	 public static String paging1(int totalRecord, int nPage, int recordPerPage,String url,int bbsno,int nowPage, String col, String word){ 
		   int pagePerBlock = 10; // 블럭당 페이지 수 
		   int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
		   int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
		   int nowGrp = (int)(Math.ceil((double)nPage/pagePerBlock));    // 현재 그룹 
...
//		     str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
	 ...
	 public static String paging2(int totalRecord, int nPage, int recordPerPage,String url,int tra_no,int nowPage, String col, String word){ 
		   int pagePerBlock = 10; // 블럭당 페이지 수 
		   int totalPage = (int)(Math.ceil((double)totalRecord/recordPerPage)); // 전체 페이지  
		   int totalGrp = (int)(Math.ceil((double)totalPage/pagePerBlock));// 전체 그룹 
		   int nowGrp = (int)(Math.ceil((double)nPage/pagePerBlock));    // 현재 그룹 
...
//		     str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		   ...
		   int _nowPage = (nowGrp-1) * pagePerBlock; // 10개 이전 페이지로 이동 
		   if (nowGrp >= 2){ 
		     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&bbsno="+tra_no+"&nPage="+_nowPage+"'>이전</A></span>"); 
		   } 
		   ...
		   for(int i=startPage; i<=endPage; i++){ 
		     ...
		     if (nPage == i){ 
		       ...
		       str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&bbsno="+tra_no+"&nPage="+i+"'>"+i+"</A></span>");   
		     } 
		   } 
		    
		   _nowPage = (nowGrp * pagePerBlock)+1; // 10개 다음 페이지로 이동 
		   if (nowGrp < totalGrp){ 
		     str.append("<span class='span_box_1'><A href='./"+url+"?nowPage="+nowPage+"&col="+col+"&word="+word+"&bbsno="+tra_no+"&nPage="+_nowPage+"'>다음</A></span>"); 
		   } 
		   str.append("</DIV>"); 
